 ...
 * only an HTTP-level test can: `server.ts#registerAdminTools` actually wires
 * `courses.listAdministered` in, over the real session/tool-call path, and
 * that a student reachable only through an enrolment (no membership) never
 * sees anything through it even though `chat.listCourses` (MCP-8) admits
 * them — the same distinction `chat-tools-server.test.ts` proves in the
 * other direction.
 */
...
describe('courses.listAdministered, over the transport (MCP-9)', () => {
  it('is wired in and lists courses across every organization the account administers', async () => {
    ...
    expect(listed.map((entry) => entry.courseId).sort()).toEqual(
      [firstCourseId, secondCourseId].sort()
    )
  })

  it('a student reachable only through an enrolment (no membership) sees nothing here, though chat.listCourses admits them', async () => {
    testDb = createTestDatabase()
