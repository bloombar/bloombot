 ...
 * helpers are not published through (`apps/worker/tests/helpers/test-db.ts`'s
 * own module comment states the same convention). Lives under `tmp/`, never
 * `data/` (QA-2, QA-3).
 */
...
const TMP_ROOT = join(process.cwd(), 'tmp', 'bot-tests')
