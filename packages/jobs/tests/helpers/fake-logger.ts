 ...
 * writing anywhere — `runner.ts` only ever calls `.info`/`.error`/`.warn` on
 * the logger it is handed, so this is all a test needs, not a real `pino`
 * instance writing under `LOGS_DIR`.
 */
