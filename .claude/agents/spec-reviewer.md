...
2. **Verification, reproduced.** Run the brief's check yourself — plus `npm run lint`,
   `npx prettier --check .`, `npm run typecheck`, `npm test` where they exist. **Do not trust reported
   output; produce your own.** If your result disagrees with what was reported, that is the most important
   finding in your review.
...
6. **The expensive mistakes**, weighted heavily when the slice touches them: tenant scoping (does every
   repo call carry an org id?), authorization (does every action declare an access policy?), migrations
   (is it additive and backward-compatible?), secrets, and student PII.

## How to report

Rank findings by severity and label each one:

- **must-fix** — correctness; tenant scoping or authorization; data loss; PII or secret exposure; a stated
  brief requirement unimplemented; a test that passes without the change.
- **cheap-fix** — duplication or naming *within files the slice already touched*.
- **note** — anything else.
