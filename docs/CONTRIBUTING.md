...
| `VITE_MCP_PUBLIC_URL` | `pages/Mcp.tsx` (WEB-47) — the MCP connector URL the tab renders, read as-is (trailing slash stripped). Set this only once a deployment has actually exposed the MCP server publicly (an nginx `location /mcp` block, or otherwise) — there is no derived fallback. | none — the tab reports the connector as not configured rather than guessing |
