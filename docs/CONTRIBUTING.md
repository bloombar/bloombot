...
| `VITE_PUBLIC_APP_URL` | `prerender-plugin.ts` — the origin `robots.txt`/`sitemap.xml` and the prerendered `/privacy`/`/terms` pages' `<link rel="canonical">` are written against. `pages/Mcp.tsx` (WEB-47) also reads it, at runtime, as the base of the MCP connector URL it renders (plus the literal `/mcp` `apps/mcp` itself answers on). | `https://bloombot.wonkledge.com` |
| `VITE_MCP_PUBLIC_URL` | `pages/Mcp.tsx` (WEB-47) — overrides the connector URL outright, for a deployment that exposes the MCP server somewhere other than `${VITE_PUBLIC_APP_URL}/mcp` (a distinct subdomain, a different path). | none — falls back to `${VITE_PUBLIC_APP_URL}/mcp`; if neither is set, the tab reports the connector as not configured rather than guessing |
