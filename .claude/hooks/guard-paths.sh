...
#
#   data/*.db        real student names, emails and conversation transcripts (PII)
#   .env, .env.*     live Discord bot token and OpenAI API key on a public repo
#   logs/*.log       a second copy of transcript content
#   results/*.csv    merged rosters — student emails and Discord handles
#
...
#
# Scoped to `data/` rather than every *.db: the test suites and Playwright
# deliberately write throwaway databases under tmp/ and e2e/tmp/, and blocking
# those would break the build instead of protecting anything.
is_protected() {
  case "$1" in
    *"/data/"*.db | data/*.db) return 0 ;;
    */.env | .env | */.env.* | .env.*) return 0 ;;
    *"/logs/"*.log | logs/*.log) return 0 ;;
    *"/results/"*.csv | results/*.csv) return 0 ;;
    *) return 1 ;;
  esac
}
...
deny() {
  echo "BLOCKED: $1" >&2
  echo "This path is protected because it holds real student PII or live credentials." >&2
...
case "$tool" in
  ...
  Bash)
    ...
    # grep, sqlite3 SELECT) is allowed: the risk here is loss and leakage
    # through modification, not inspection.
    #
    ...
    if [ -n "$command" ]; then
      ...
      case "$first_line" in
        rm\ * | *\;\ rm\ * | *"&& rm "* | *"| rm "* | *truncate\ * | *shred\ *)
          case "$first_line" in
            *data/*.db* | *.env | *.env\ * | *.env.* | *logs/*.log* | *results/*.csv*)
      ...
      # Redirection into a protected path (`> data/data.db`, `>> .env`).
      case "$first_line" in
        *">"*data/*.db* | *">"*/.env* | *">"\ .env* | *">"*logs/*.log*)
