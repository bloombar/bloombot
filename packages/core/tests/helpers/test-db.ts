 ...
 * `packages/legacy-import/tests/helpers/platform-db.ts` use, duplicated here
 * rather than imported across a package boundary test helpers are not
 * published through. Lives under `tmp/`, never `data/` (QA-2, QA-3).
 */
...
const TMP_ROOT = join(process.cwd(), 'tmp', 'core-tests')
