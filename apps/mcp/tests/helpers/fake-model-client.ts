/**
 * A stand-in `ModelClient` (`@bloombot/core`'s `ports.ts`) for this app's own
 * tests — no network, an answer fixed at construction. The same shape
 * `apps/api/tests/helpers/fake-model-client.ts` and
 ...
 * field, the same reasoning those files' own module comments give for not
 * sharing a single copy across the app/package boundary.
 */
