 ...
 * Mirrors `packages/db/tests/client.test.ts`'s own identical helper (D-91);
 ...
 * timeout instead (`client.test.ts`'s own D-91 rework fixed exactly this).
...
/**
 * D-91 rework round 1's own hot-path concern, reproduced directly:
 * `signInWithGoogle` reads (`accountsRepo.getAccountByEmail`) before it
 * writes (creating the account, the session, or both), the exact
 * deferred-upgrade shape D-91 exists to close, on the path two concurrent
 ...
 */
describe('signInWithGoogle: a concurrent writer elsewhere is waited for, not raced (D-91)', () => {
