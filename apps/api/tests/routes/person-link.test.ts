 ...
 *
 * The last `describe` block is this slice's own acceptance test — the
 * brief's own "write at least one test that starts where a real person
 * starts": a student admitted by a roster import, who has never signed in,
 * sends a Discord message (simulated the same way `chat.test.ts`'s own
 ...
 * gets the LINK-1 invitation, follows it, signs in, connects through this
...
describe('POST /organizations/:organizationId/person-link/discord/begin', () => {
  ...
  it('returns a scope=identify authorization URL, and creates the caller a connected web person in that organization', async () => {
    testDb = createTestDatabase()
    const caller = seedSignedInCaller(testDb.db)
    const app = await buildTestApp(testDb.db)
...
describe('POST .../discord/preview and .../discord/confirm', () => {
  it('previews an attach outcome without consuming the state — confirm still succeeds afterward', async () => {
    testDb = createTestDatabase()
    const caller = seedSignedInCaller(testDb.db)
  ...
  it("confirm never trusts a client-resupplied discordExternalId — only what preview's own exchange actually proved", async () => {
    testDb = createTestDatabase()
    const caller = seedSignedInCaller(testDb.db)
    const fakeDiscord = createFakeDiscordRestClient({
    ...
    // A confirm body has no field for this at all — only `state` — so there
    // is no way for a caller to assert a different identity than the one
    // `/discord/preview` already proved.
    await request(app)
      .post(
        `/organizations/${caller.organizationId}/person-link/discord/confirm`
      )
...
describe('POST .../mcp/preview and .../mcp/confirm (LINK-8)', () => {
  it('previews then confirms an MCP-issued token, attaching the identity to the caller own connected person', async () => {
    ...
    const caller = seedSignedInCaller(testDb.db)
    const app = await buildTestApp(testDb.db)
    const { issueMcpPersonLinkToken } = await import('@bloombot/auth')
...
describe('acceptance — a roster-admitted student who has never signed in connects Discord and reaches the web chat', () => {
  it('follows the LINK-1 invitation, signs in, connects through the real HTTP endpoints, then asks a question and gets an answer', async () => {
    testDb = createTestDatabase()
    ...
    // The student signs in — a brand-new account, no relationship yet to
    // the organization their Discord identity already belongs to (the
    // account this router's own `ensureWebPersonForAccount` has to create a
    // person for on demand).
