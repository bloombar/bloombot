 ...
 * identical reason).
 *
 * **WEB-69 — the Danger zone moved out.** This screen used to end with the
 * organization's own delete control (WEB-72/DATA-7); it is now
 * `components/DangerZone.tsx`, its own tab on `pages/OrganizationSettings.tsx`
 * rather than the bottom of this one (`docs/DECISIONS.md` has the reasoning)
 * — this component no longer takes `organizationName`/`navigate`/
 * `refreshAccount` at all, since nothing left here needs them.
 *
 * **WEB-69 — this tab's own dirtiness is reported up, not tracked with its
 * own navigation guard**, the identical reasoning
 * `components/CourseInstructions.tsx`'s own module comment already gives for
 * the same shape one level up: `pages/OrganizationSettings.tsx` folds every
 * tab's own dirty flag into the one prompt it shows before a tab switch or a
 * leave, so a second, independently-registered guard here would only
 * clobber that one. Two things can make this tab dirty — this file's own
 * grant form (a non-blank email typed but not yet granted) and the nested
 * `components/MembershipInvitations.tsx`'s own pending invite — folded
 * together into one `onDirtyChange`/`onRegisterActions` pair the same way
 * `pages/CourseEditor.tsx` folds this file's own instructions section one
 * level up: `handleTeamSave`, below, saves whichever half is actually dirty
 * (or both), and `handleTeamDiscard` resets whichever half is.
 */
...
export interface TeamProps {
  ...
  /**
   * WEB-69 — called on every change to whether this tab holds an unsaved
   * edit: either half — the grant form's own pending email, or the nested
   * `MembershipInvitations`' own pending invite — being non-blank, folded
   * into one flag the same way `pages/CourseEditor.tsx` folds
   * `components/CourseInstructions.tsx`'s own dirtiness into its one
   * `isDirty`. Optional, defaulting to a no-op — most of this file's own
   * tests do not care.
   */
  onDirtyChange?: (dirty: boolean) => void
  /** WEB-69 — this tab's own save/discard handle, saving (or discarding) whichever half is actually dirty — the same `onRegisterActions` shape `pages/Usage.tsx` already exposes; called with `null` on unmount. Optional — most of this file's own tests do not care. */
...
export function Team({
  ...
  // WEB-69 — read synchronously by this tab's own `isSaving`, the same
  // reason `pages/Usage.tsx`'s own `savingRef` exists.
  ...
  // WEB-69 — the nested `MembershipInvitations`' own dirty flag/actions,
  // registered up to this component the same way it registers its own up to
  // `pages/OrganizationSettings.tsx` (`handleRegisterInvitationActions`,
  // below) — `null` until that section has mounted (it always has, once
  // this tab is visited: unlike the course editor's own sections, this one
  // is never conditionally gated).
  const [invitationsDirty, setInvitationsDirty] = useState(false)
  const invitationActionsRef = useRef<TabDirtyActions | null>(null)
  const handleRegisterInvitationActions = useCallback(
    (actions: TabDirtyActions | null) => {
      invitationActionsRef.current = actions
    },
    []
  )
  ...
  // WEB-69: returns whether the role actually granted, so a caller that
  // saves on the way somewhere else (`handleTeamSave`, below) knows whether
  // it is safe to move on — the same `Promise<boolean>` shape
  // `pages/Usage.tsx#handleSave` already returns for the identical reason.
  const handleGrant = useCallback(async (): Promise<boolean> => {
  ...
  }, [email, role, organizationId, confirm, refresh])
  ...
  // WEB-69 — "dirty" for this tab: this file's own grant form holds a
  // non-blank, not-yet-granted email, or the nested `MembershipInvitations`
  // does (this file's own module comment on why both halves fold into one
  // flag).
  const grantDirty = email.trim() !== ''
  const isDirty = grantDirty || invitationsDirty

  useEffect(() => {
    onDirtyChange(isDirty)
  }, [isDirty, onDirtyChange])

  // WEB-69 — saves whichever half is actually dirty (or both); the nested
  // `MembershipInvitations` first, then this file's own grant form — the
  // same "each half is its own action with its own failure" reasoning
  // `pages/CourseEditor.tsx#saveDirtyWork` already holds itself to for its
  // own two independent halves, one level up.
  const handleTeamSave = useCallback(async (): Promise<boolean> => {
    if (invitationsDirty) {
      const actions = invitationActionsRef.current
    ...
    return true
  }, [invitationsDirty, grantDirty, handleGrant])

  const handleTeamDiscard = useCallback(() => {
    setEmail('')
    setGrantError(undefined)
    invitationActionsRef.current?.discard()
  }, [])
  ...
  useEffect(() => {
    ...
    onRegisterActions({
      ...
      isSaving: () =>
        grantingRef.current ||
        (invitationActionsRef.current?.isSaving() ?? false),
      discard: handleTeamDiscard,
    })
    return () => onRegisterActions(null)
  }, [handleTeamSave, handleTeamDiscard, onRegisterActions])
      ...
      {isOwner && (
        <div className="border-t border-neutral-200 pt-4">
          <MembershipInvitations
            ...
            onDirtyChange={setInvitationsDirty}
            onRegisterActions={handleRegisterInvitationActions}
          />
        </div>
      )}
    </div>
