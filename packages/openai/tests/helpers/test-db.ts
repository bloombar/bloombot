 ...
 * test file — the same shape `packages/core/tests/helpers/test-db.ts` uses,
...
const TMP_ROOT = join(process.cwd(), 'tmp', 'openai-tests')
...
export function createTestDatabase(): TestDatabase {
  ...
  return {
    ...
    cleanup: () => {
      closeDatabase(db)
      // SQLite in WAL mode writes `-wal` and `-shm` siblings alongside the
      // main file; all three need to go.
